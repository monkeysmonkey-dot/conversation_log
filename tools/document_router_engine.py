...
def route(doc):
    ...
    suggestion = doc.get("suggestion", {})
    verification_required = bool(suggestion.get("verification_required", True))
    source_trust = suggestion.get("source_trust", "unknown")
    evidence_use = suggestion.get("evidence_use", "lead_only")
    required_agent_action = suggestion.get("required_agent_action", "Verify before use.")

    if verification_required:
        status = "verification_required"
        action = "Verify source/facts before routing or applying."
    ...
    return {
        ...
        "can_apply_without_user_confirmation": False
    }
