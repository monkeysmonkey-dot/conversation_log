...
# ============================================================
# Hermes Universal Floating Upload Drop Box
# ============================================================

def render_hermes_universal_upload_button():
    import json as _hermes_upload_json
    import subprocess as _hermes_upload_subprocess
    from datetime import datetime as _hermes_upload_datetime, timezone as _hermes_upload_timezone
    ...
    def _now_utc():
        return _hermes_upload_datetime.now(_hermes_upload_timezone.utc).isoformat()

    def _load_json_local(path, default):
        try:
            return _hermes_upload_json.loads(path.read_text(encoding="utf-8"))
        except Exception:
            return default

    def _save_json_local(path, data):
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(_hermes_upload_json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
...
.hermes-floating-upload-title {
        ...
        padding: 0.95rem 1.0rem 0.70rem 1.0rem;
    ...
    /* When quick upload is open, turn the next Streamlit file uploader into a floating drop box */
    body:has(.hermes-floating-upload-title) div[data-testid="stFileUploader"] {
        position: fixed !important;
        top: 17.35rem !important;
    ...
    <a class="hermes-upload-circle" href="?quick_upload=1" title="Upload documents">↥</a>
    """, unsafe_allow_html=True)
...
<div class="hermes-floating-upload-title">
            ...
            <span>Drag and drop files here, or click Browse. Files go to Document Inbox for sorting confirmation.</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if _quick_files:
            _manifest = _load_json_local(_document_manifest, {"documents": []})
            ...
            _save_json_local(_document_manifest, _manifest)

            try:
                _hermes_upload_subprocess.run(
...
render_hermes_universal_upload_button()
