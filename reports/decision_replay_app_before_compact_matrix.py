...
def build_replay_matrix(replay, price_snapshot):
    ...
    for ticker in tickers:
        ...
        rows.append({
            "Ticker": ticker,
            "Manual": readiness_icon(manual.get("replay_ready")) if manual else "—",
            "Agent": readiness_icon(agent.get("replay_ready")) if agent else "—",
            "Price": price_verified_icon(price_item),
            "Thesis": "✅" if thesis else "—",
            "Suggestion": suggestion_icon(agent.get("suggestion_type")) if agent else "",
            "Manual Ref": manual_ref,
            "Agent Ref": agent_ref,
            "Current": current_price,
            "Move": fmt_pct(manual.get("movement_pct")) if manual else fmt_pct(agent.get("movement_pct")) if agent else "",
            "Price Check": price_status_short(price_item),
            "Status": clean_status(manual, "manual") if manual else clean_status(agent, "agent") if agent else "No replay",
...
with tab_matrix:
    ...
    l1.caption("✅ ready / verified")
    l2.caption("❌ blocked / missing")
    l3.caption("⚠️ needs review")
    l4.caption("⏳ wait for confirmation")
