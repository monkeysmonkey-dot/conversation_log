...
PRICE_SNAPSHOT = BASE / "features" / "latest_price_snapshot.json"
...
def price_verification_map():
    packet = load_json(PRICE_SNAPSHOT, {})
    out = {}
...
def batch_price_review_rows(batch_df):
    price_map = price_verification_map()
    rows = []
    ...
    for _, row in batch_df.iterrows():
        ...
        try:
            entered_price = float(row.get("current_price", 0) or 0)
        except Exception:
            entered_price = 0.0

        verified_item = price_map.get(ticker, {})
        verified_price = verified_item.get("selected_price", None)
        verified_status = str(verified_item.get("status", "")).replace("_", " ")

        try:
            verified_price_float = float(verified_price or 0)
        except Exception:
            verified_price_float = 0.0

        diff_pct = None
        status = "⚠️ No verified price"
        suggested = "Run price verification or confirm manually."

        if entered_price <= 0:
            status = "❌ Missing entered price"
            suggested = "Enter a current price greater than zero."
        elif verified_price_float > 0:
            diff_pct = ((entered_price - verified_price_float) / verified_price_float) * 100

            if abs(diff_pct) <= 1.0:
                status = "✅ Matches"
                suggested = "Safe to use."
            elif abs(diff_pct) <= 3.0:
                status = "⚠️ Review"
                suggested = "Close, but review before saving."
            else:
                status = "❌ Disagrees"
                suggested = "Fix entered price before saving."

        rows.append({
            "Ticker": ticker,
            "Entered Price": round(entered_price, 4) if entered_price else "",
            "Verified Price": round(verified_price_float, 4) if verified_price_float else "",
            "Difference": f"{diff_pct:+.2f}%" if diff_pct is not None else "",
            "Verification": verified_status if verified_status else "not available",
            "Status": status,
...
with tab_batch:
    ...
    st.markdown("### Price Review Before Save")
    st.caption("Entered current prices are checked against verified external prices when available.")

    review_rows = batch_price_review_rows(edited_df)

    if review_rows:
        st.dataframe(review_rows, width="stretch", hide_index=True)

        severe_count = sum(1 for row in review_rows if str(row.get("Status", "")).startswith("❌"))
        warning_count = sum(1 for row in review_rows if str(row.get("Status", "")).startswith("⚠️"))

        if severe_count > 0:
            st.error(f"{severe_count} price issue(s) need correction before replay is reliable.")
        elif warning_count > 0:
            st.warning(f"{warning_count} price warning(s) should be reviewed.")
        else:
            st.success("Entered prices match verified prices within tolerance.")
    else:
        st.caption("No tickers available for price review.")
    ...
    with b3:
        if st.button("Run Batch Replay Update", width="stretch"):
            ...
            if severe_count > 0:
                st.warning(f"{severe_count} price issue(s) detected. Batch will still run, but replay may be unreliable until corrected.")
