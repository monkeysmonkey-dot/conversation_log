...
# ============================================================
# Hermes Legacy Body Button Filter
# ============================================================

_HERMES_HIDE_BUTTON_LABELS = {
    "Build Queue",
    "Open One Source",
    "Close Browser",
    "Open Decision Entry App",
    "Refresh Trends",
    "Refresh System Status",
    "Refresh Decision Data Status",
    "Refresh Decision Data + Replay",
    "Refresh Decision Data",
    "Refresh Replay",
    "Refresh Mutual Fund Review",
    "Refresh Price Verification",
    "Refresh Tax Advisor",
}

if not getattr(st, "_hermes_legacy_button_filter_installed", False):
    _hermes_original_st_button = st.button

    def _hermes_filtered_st_button(label, *args, **kwargs):
        if str(label) in _HERMES_HIDE_BUTTON_LABELS:
            return False
        return _hermes_original_st_button(label, *args, **kwargs)

    st.button = _hermes_filtered_st_button

    try:
        from streamlit.delta_generator import DeltaGenerator as _HermesDeltaGenerator

        _hermes_original_delta_button = _HermesDeltaGenerator.button

        def _hermes_filtered_delta_button(self, label, *args, **kwargs):
            if str(label) in _HERMES_HIDE_BUTTON_LABELS:
                return False
            return _hermes_original_delta_button(self, label, *args, **kwargs)

        _HermesDeltaGenerator.button = _hermes_filtered_delta_button
    except Exception:
        pass

    st._hermes_legacy_button_filter_installed = True
...
# ============================================================
# Hermes Fixed Corner Refresh / Upload Controls
# ============================================================

def render_hermes_fixed_corner_controls():
    ...
    _document_inbox = BASE / "data" / "document_inbox"
    _document_uploads = _document_inbox / "uploads"
    _document_manifest = _document_inbox / "manifest.json"
    ...
    def _run_hidden(cmd, timeout=240):
        if cmd and cmd[0] == "py":
    ...
    def _refresh_current_app():
        _app_name = _Path(__file__).name

        if _app_name == "mobile_console_app.py":
            _cmds = [
                ["py", "tools\\fx_rate_engine.py"],
                ["py", "tools\\document_intake_engine.py"],
                ["py", "tools\\document_router_engine.py"],
                ["py", "tools\\market_console_trend_engine.py"],
                ["py", "tools\\mutual_fund_analysis_engine.py"],
                ["py", "tools\\mutual_fund_review_scheduler.py"],
                ["py", "tools\\tax_advisor_engine.py"],
                ["py", "tools\\system_status_engine.py"],
            ]
        elif _app_name == "decision_entry_app.py":
            _cmds = [
                ["py", "tools\\fx_rate_engine.py"],
                ["py", "tools\\document_intake_engine.py"],
                ["py", "tools\\document_router_engine.py"],
                ["py", "tools\\sync_account_portfolio_snapshot.py"],
                ["py", "tools\\price_fallback_engine.py"],
        ...
        elif _app_name == "decision_replay_app.py":
            _cmds = [
                ["py", "tools\\fx_rate_engine.py"],
                ["py", "tools\\sync_account_portfolio_snapshot.py"],
                ["py", "tools\\price_fallback_engine.py"],
            ...
            _cmds = [["py", "tools\\system_status_engine.py"]]

        _ok = 0
        for _cmd in _cmds:
            try:
                _result = _run_hidden(_cmd, timeout=240)
                if _result.returncode == 0:
                    _ok += 1
            except Exception:
                pass

        return _ok, len(_cmds)

    try:
        _refresh_requested = st.query_params.get("hermes_refresh", "")
        _quick_upload_open = st.query_params.get("quick_upload", "")
    except Exception:
        _refresh_requested = ""
        _quick_upload_open = ""

    if _refresh_requested == "1":
        with st.spinner("Refreshing current Hermes page..."):
            _ok, _total = _refresh_current_app()

        st.toast(f"Refresh complete: {_ok}/{_total} tasks finished.")
        st.query_params["hermes_refresh"] = "0"
        st.rerun()

    _upload_href = "?quick_upload=0" if _quick_upload_open == "1" else "?quick_upload=1"

    st.markdown(f"""
    <style>
    .hermes-fixed-control {{
        position: fixed;
        right: 1.05rem;
        z-index: 2147483646;
    ...
    .hermes-fixed-control:hover {{
        background: rgba(31, 41, 55, 0.98);
        border-color: rgba(156, 163, 175, 1.0);
        transform: translateY(-1px);
    }}

    .hermes-fixed-refresh {{
        top: 6.95rem;
    }}
    ...
    .hermes-floating-upload-title {{
        ...
        padding: 1.20rem 1.30rem 1.00rem 1.30rem;
    ...
    .hermes-floating-upload-title strong {{
        color: white;
        font-size: 1.10rem;
    }}

    .hermes-floating-upload-title span {{
        display: block;
        margin-top: 0.40rem;
    ...
    body:has(.hermes-floating-upload-title) div[data-testid="stFileUploader"] {{
    ...
    body:has(.hermes-floating-upload-title) div[data-testid="stFileUploader"] section {{
        min-height: 7.25rem !important;
        border: 1px dashed rgba(147, 197, 253, 0.75) !important;
        border-radius: 0.85rem !important;
        background: rgba(30, 41, 59, 0.72) !important;
    }}
    </style>

    <a class="hermes-fixed-control hermes-fixed-refresh"
       href="?hermes_refresh=1"
       target="_self"
       onclick="window.location.href='?hermes_refresh=1'; return false;"
       title="Refresh current page">⟳</a>

    <a class="hermes-fixed-control hermes-fixed-upload"
       href="{_upload_href}"
       target="_self"
       onclick="window.location.href='{_upload_href}'; return false;"
       title="Quick upload">↥</a>
    """, unsafe_allow_html=True)

    if _quick_upload_open == "1":
        st.markdown("""
        <div class="hermes-floating-upload-title">
            <strong>Quick Upload</strong>
            <span>Drag and drop files here, or click Browse. Files are saved into Document Inbox and wait for your sorting confirmation.</span>
        </div>
        """, unsafe_allow_html=True)

        _document_uploads.mkdir(parents=True, exist_ok=True)

        _quick_files = st.file_uploader(
            "Drop files here",
            accept_multiple_files=True,
            type=["pdf", "csv", "txt", "md", "json", "xlsx", "xls", "png", "jpg", "jpeg"],
            key=f"hermes_fixed_upload_{_Path(__file__).stem}"
        )

        if _quick_files:
            _manifest = _load_json(_document_manifest, {"documents": []})
            _docs = _manifest.get("documents", [])
            _existing_names = set([d.get("filename") for d in _docs])

            _added = 0

            for _uploaded in _quick_files:
                _safe_name = _uploaded.name.replace("/", "_").replace("\\\\", "_")
                _target = _document_uploads / _safe_name
                _target.write_bytes(_uploaded.getbuffer())

                if _safe_name not in _existing_names:
                    _docs.append({
                        "filename": _safe_name,
                        "path": str(_target),
            ...
            _manifest["documents"] = _docs
            _manifest["updated_at"] = _now_utc()
            _save_json(_document_manifest, _manifest)

            try:
                _run_hidden(["py", "tools\\document_intake_engine.py"], timeout=180)
            except Exception:
                pass

            st.success(f"Uploaded {len(_quick_files)} file(s). New documents added: {_added}. Confirm sorting in Document Inbox.")


render_hermes_fixed_corner_controls()
...
def build_replay_matrix(replay, price_snapshot):
    ...
    for ticker in tickers:
        ...
        agent_model_clean = agent_model
        agent_model_clean = agent_model_clean.replace("decision_entry_batch_replay", "batch")
        agent_model_clean = agent_model_clean.replace("manual_input", "manual")
        ...
        rows.append({
            ...
            "Thesis": thesis_short,
            "Signal": signal,
            "Agent / Model": agent_model_clean,
            "Entry": ref_value,
            "Current": current_price,
            "Move": move_value,
            "Target": exp_move,
            "Stop": stop_loss,
            "Price OK": price_status_short(price_item),
    ...
    if rows:
        st.dataframe(
            ...
            column_config={
                ...
                "Price OK": st.column_config.TextColumn(
                    "Price OK",
                ...
                "Entry": st.column_config.TextColumn(
                    "Entry",
                    help="Thesis entry/reference price used to measure replay performance."
                ),
                ...
                "Price OK": st.column_config.TextColumn(
                    "Price OK",
                    help="Price verification status. Verified, warning, or blocked."
                ),
