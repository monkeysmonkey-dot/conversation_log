...
# ============================================================
# Hermes Native In-Window Controls
# ============================================================

def render_hermes_native_in_window_controls():
    ...
    def _refresh_current_app():
        ...
        for _cmd in _cmds:
            try:
                _r = _run_hidden(_cmd, timeout=240)
                if _r.returncode == 0:
                    _ok += 1
            except Exception:
                pass
    ...
    st.markdown("""
    <style>
    /* Native control panel: no anchor links, no new windows */
    .hermes-native-controls-note {
    ...
    top_space, refresh_col, upload_col = st.columns([11.0, 0.55, 0.55])

    with refresh_col:
        if st.button("⟳", key=f"hermes_native_refresh_{_Path(__file__).stem}", help="Refresh this Hermes page", use_container_width=True):
            with st.spinner("Refreshing this page..."):
    ...
    with upload_col:
        try:
            pop = st.popover("↥", help="Quick upload", use_container_width=True)
        except Exception:
            pop = st.expander("↥ Upload", expanded=False)

        with pop:
            st.markdown("### Quick Upload")
            st.caption("Drag and drop files here, or click Browse. Files are saved into Document Inbox and wait for your sorting confirmation.")
            ...
            _quick_files = st.file_uploader(
                ...
                key=f"hermes_native_upload_{_Path(__file__).stem}"
            )
...
render_hermes_native_in_window_controls()
